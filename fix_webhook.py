...
# Fix the broken formatQuotationResults function
new_formatter = r"""function formatQuotationResults(data: any): string {
  if (!data) return "";
...
# The previous script broke the file by failing at the regex/replacement.
# Let's find the start of formatQuotationResults and replace until the next function definition or reasonable end.
content = re.sub(r'function formatQuotationResults\(data: any\): string \{.*?Qual dessas opções faz mais sentido para você\? Ou prefere aguardar o consultor com as datas exatas\? 😊";\s+return formatted\.trim\(\);\s+\}.*?async function createQuoteRequest', new_formatter + '\n\nasync function createQuoteRequest', content, flags=re.DOTALL)
