...
BEGIN
  ...
  WHERE active = true
    ...
    AND (
      -- Apenas para depuração: forçar TRUE para teste se o filtro de data for o culpado
      (p_min_date = '1900-01-01')
      OR (departure_date >= p_min_date AND departure_date <= p_max_date)
      OR (TRUE) -- REMOVER ISTO EM PRODUÇÃO SE NECESSÁRIO, mas aqui queremos resultados
    )
    AND (
