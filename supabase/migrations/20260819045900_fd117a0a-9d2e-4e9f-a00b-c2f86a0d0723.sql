...
BEGIN
  ...
  IF v_dest_normalized LIKE '%maceio%' THEN v_dest_terms := v_dest_terms || ARRAY['mcz']; END IF;
  IF v_dest_normalized LIKE '%porto de galinhas%' THEN v_dest_terms := v_dest_terms || ARRAY['rec', 'recife']; END IF;
  IF v_dest_normalized LIKE '%natal%' THEN v_dest_terms := v_dest_terms || ARRAY['nat']; END IF;
  IF v_dest_normalized LIKE '%fortaleza%' THEN v_dest_terms := v_dest_terms || ARRAY['for']; END IF;
  IF v_dest_normalized LIKE '%salvador%' THEN v_dest_terms := v_dest_terms || ARRAY['ssa']; END IF;
  ...
  IF p_origin_term IS NOT NULL THEN
    ...
    END IF;
    IF v_origin_normalized LIKE '%goiania%' THEN v_origin_terms := v_origin_terms || ARRAY['gyn']; END IF;
    IF v_origin_normalized LIKE '%porto alegre%' THEN v_origin_terms := v_origin_terms || ARRAY['poa']; END IF;
    IF v_origin_normalized LIKE '%curitiba%' THEN v_origin_terms := v_origin_terms || ARRAY['cwb']; END IF;
    ...
    IF v_origin_normalized LIKE '%brasilia%' THEN v_origin_terms := v_origin_terms || ARRAY['bsb']; END IF;
  END IF;
