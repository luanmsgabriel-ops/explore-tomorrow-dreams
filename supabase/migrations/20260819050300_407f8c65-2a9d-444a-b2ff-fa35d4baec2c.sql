...
BEGIN
  ...
  WHERE active = true
    ...
    AND (
      ...
      OR (v_dest_clean = 'maceio' AND (lower(destination_name) LIKE '%mcz%' OR lower(destination_iata) = 'mcz'))
      OR (v_dest_clean = 'porto de galinhas' AND (lower(destination_name) LIKE '%rec%' OR lower(destination_iata) = 'rec'))
    )
    AND (
      p_origin_term IS NULL
      OR public.unaccent(lower(origin_city)) LIKE '%' || v_origin_clean || '%'
      OR lower(origin_iata) = lower(p_origin_term)
      OR (v_origin_clean = 'sao paulo' AND (lower(origin_city) IN ('campinas', 'guarulhos', 'congonhas') OR lower(origin_iata) IN ('gru', 'cgh', 'vcp')))
      OR (v_origin_clean = 'goiania' AND (lower(origin_city) LIKE '%goiania%' OR lower(origin_iata) = 'gyn'))
    )
  ORDER BY 
