...
serve(async (req) => {
  ...
  try {
    ...
    const payloadIdx = html.indexOf("PAYLOAD = {");
    const pvooIdx = html.indexOf("__PVOO_PAYLOAD = {");
    const startIdx = payloadIdx !== -1 ? payloadIdx : (pvooIdx !== -1 ? pvooIdx : -1);
    ...
    const payloadBlock = html.substring(startIdx, startIdx + 150000);
    const mapaRawMatch = payloadBlock.match(/mapa\s*:\s*({[\s\S]*?}),\s*usd/i);
    const blobRawMatch = payloadBlock.match(/blob\s*:\s*[`"']([\s\S]*?)[`"']/i);
    const usdValueMatch = payloadBlock.match(/usd\s*:\s*([\d.]+)/i);

    if (!mapaRawMatch || !blobRawMatch) {
       return new Response(JSON.stringify({ 
         status: "error", 
         message: "Mapa or Blob missing in PAYLOAD block",
         payload_block_sample: payloadBlock.substring(0, 1000)
       }), { status: 200, headers: corsHeaders });
    }

    const mapaRaw = mapaRawMatch[1];
    const blobRaw = blobRawMatch[1];
    ...
    const packageOffers = [];
    if (snapshotIdx !== -1) {
      const snapshotBlock = html.substring(snapshotIdx, snapshotIdx + 100000);
      const objBlocks = snapshotBlock.match(/{[\s\S]*?}/g) || [];
        ...
        try {
          ...
          if (dataStr && dataStr.includes(' a ')) {
            ...
            const parseBR = (s: string) => {
              ...
              return dParts.length === 3 ? `${dParts[2]}-${dParts[1]}-${dParts[0]}` : null;
            };
          ...
          packageOffers.push({
            ...
            source_id: sourceId,
            offer_type: 'pacote',
            origin_city: getValue('origem_cidade'),
            ...
            raw_data: { uf: getValue('uf'), min_label: getValue('min_label'), fonte, original_name: nome },
            last_seen_at: executionTimestamp,
            active: true
          });
          stats.pacoteCount++;
        } catch (e) { stats.errors.push(`Erro pacote: ${e.message}`); }
      }
    }
    ...
    return new Response(JSON.stringify({ status: "success", stats }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err: any) {
    return new Response(JSON.stringify({ error: err.message, stats }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});