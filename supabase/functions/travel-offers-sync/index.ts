...
serve(async (req) => {
  ...
  try {
    ...
    const payloadMatch = html.match(/(?:const|var|let)\s+(?:PAYLOAD|__PVOO_PAYLOAD)\s*=\s*({[\s\S]*?});/i) ||
                         html.match(/(?:PAYLOAD|__PVOO_PAYLOAD)\s*=\s*({[\s\S]*?})\s*,?\n/i);
    ...
    let blobRaw = payloadContent.match(/blob\s*:\s*[`"']([\s\S]*?)[`"']/i)?.[1];

    if (!mapaRaw || !blobRaw) {
       return new Response(JSON.stringify({ 
         status: "error", 
         message: "Mapa or Blob missing in PAYLOAD",
         has_mapa: !!mapaRaw,
         has_blob: !!blobRaw,
         payload_sample: payloadContent.substring(0, 1000)
       }), { status: 200, headers: corsHeaders });
    }
    ...
    if (snapshotMatch) {
      ...
      for (const block of objBlocks) {
        try {
          ...
          stats.errors.push(`Erro pacote: ${e.message}`);
        }
      }
    }
