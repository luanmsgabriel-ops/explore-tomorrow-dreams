...
function cleanProfile(input: unknown) {
  ...
  const directFlightPreference = typeof value.directFlightPreference === "string" && allowedDirectFlight.has(value.directFlightPreference)
    ? value.directFlightPreference
    : "neutral";
  const lodgingPreferences = Array.isArray(value.lodgingPreferences)
    ? value.lodgingPreferences.filter((item): item is string => typeof item === "string").map((item) => item.trim()).filter(Boolean).slice(0, 12)
    : [];
  ...
  if (budgetMin !== null && budgetMin < 0) throw new Error("invalid_budget_range");
  if (budgetMax !== null && budgetMax < 0) throw new Error("invalid_budget_range");
  if (budgetMin !== null && budgetMax !== null && budgetMin > budgetMax) throw new Error("invalid_budget_range");

  return {
    home_origin_name: cleanText(value.homeOriginName),
    home_origin_iata: homeOriginIata,
    typical_party: typicalParty,
    budget_min: budgetMin,
    budget_max: budgetMax,
    budget_currency: cleanText(value.budgetCurrency, 3)?.toUpperCase() ?? "BRL",
    direct_flight_preference: directFlightPreference,
    lodging_preferences: lodgingPreferences,
    onboarding_completed_at: value.onboardingCompleted === true ? new Date().toISOString() : null,
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "get") {
      const [profileResult, affinitiesResult, eventsResult] = await Promise.all([
      ...
      for (const event of eventsResult.data ?? []) {
        if (!latestAnswers[event.preference_key]) latestAnswers[event.preference_key] = event.response;
      }
      return json({ ok: true, profile: profileResult.data ?? null, affinities: affinitiesResult.data ?? [], latestAnswers });
    }

    if (action === "update_profile") {
      const profile = cleanProfile(body?.profile);
      const { data, error } = await client.from("traveler_profile_settings")
    ...
    if (action === "answer") {
      const preferenceKey = cleanText(body?.preferenceKey, 80) ?? "";
    ...
    if (["invalid_home_origin_iata", "invalid_budget_range"].includes(message)) return json({ ok: false, error: message }, 400);
    console.error("[MY_TOMORROW_PROFILE_ERROR]", message);
    return json({ ok: false, error: "travel_profile_request_failed" }, 500);
  }
});
