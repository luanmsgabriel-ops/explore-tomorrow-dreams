...
const FOUNDATION_INSTRUCTIONS = [
  ...
  "Antes de iniciar qualquer busca de pacote ou bloqueio, confirme a origem de saída. Nunca pesquise sem uma origem informada e confirmada pelo cliente.",
  "Também confirme o período antes da busca. A única exceção é quando o cliente disser explicitamente que aceita qualquer data, todo o período disponível, todo o inventário ou quer o menor preço independentemente da data.",
  "Se o cliente informar apenas a cidade onde está, não presuma silenciosamente um aeroporto. Só apresente aeroportos próximos quando eles vierem de uma fonte ou ferramenta verificada disponível na sessão; se não houver essa fonte, peça ao cliente que confirme o aeroporto de saída. Nunca invente aeroporto, IATA ou distância.",
  ...
  "Se o cliente pedir comparação entre destinos diferentes, limite a comparação a no máximo três destinos por rodada. Faça uma busca separada por destino e, para cada destino, apresente somente a oportunidade de menor preço encontrada dentro dos filtros confirmados.",
  "Numa comparação de destinos, nunca apresente três hotéis ou três datas do mesmo destino. O resultado final deve ter exatamente um card por destino solicitado, no máximo três cards no total.",
...
const TRAVEL_OFFERS_TOOL = {
  ...
  description: [
    ...
    "A origem precisa estar confirmada antes da chamada. O período também precisa estar confirmado, exceto quando o cliente autorizar explicitamente qualquer data ou todo o inventário.",
    "Para comparar destinos diferentes na mesma fala, faça chamadas separadas por destino, no máximo três. A aplicação fará a curadoria visual de uma oferta de menor preço por destino.",
  ...
  parameters: {
    ...
    properties: {
      ...
      origin: {
        type: "string",
        description: "Cidade ou aeroporto de origem confirmado pelo cliente.",
      },
      destination: {
        type: "string",
      ...
      start_date: {
        type: "string",
        description: "Data inicial de saída no formato YYYY-MM-DD. Omitir somente quando o cliente autorizou explicitamente qualquer data ou todo o inventário.",
      },
      end_date: {
        type: "string",
        description: "Data final de saída no formato YYYY-MM-DD. Omitir somente quando o cliente autorizou explicitamente qualquer data ou todo o inventário.",
      },
      passengers: {
        type: "integer",
        minimum: 1,
