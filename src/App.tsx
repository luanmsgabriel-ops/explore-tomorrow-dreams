...
const MyTomorrowWelcome = lazyWithRetry(() => import("./pages/MyTomorrowWelcome"));
...
const MyTomorrowNotifications = lazyWithRetry(() => import("./pages/MyTomorrowNotifications"));
...
const SmoothScroll = () => { useLenis(); return null; };
const PageSuspense = ({ children, label, opportunities = false }: { children: ReactNode; label: string; opportunities?: boolean }) => <Suspense fallback={<div className={opportunities ? "min-h-screen bg-[#041012]" : "min-h-screen bg-background"} aria-label={`Carregando ${label}`} />}>{children}</Suspense>;
const ClientProtected = ({ children, label }: { children: ReactNode; label: string }) => <ClientAuthGuard><PageSuspense label={label}>{children}</PageSuspense></ClientAuthGuard>;
const FloatingButtons = () => { const location = useLocation(); const hideOnRoutes = ['/cliente', '/minha-area', '/admin', '/admin/dashboard', '/avaliacao', '/experiencia', '/oportunidades']; if (hideOnRoutes.some(route => location.pathname.startsWith(route))) return null; return <Suspense fallback={null}><TravelAdvisorChat /></Suspense>; };

const App = () => (
  <QueryClientProvider client={queryClient}><TooltipProvider><Toaster /><Sonner /><BrowserRouter><SmoothScroll /><Routes>
    ...
    <Route path="/minha-area/boas-vindas" element={<ClientProtected label="Travel Match"><MyTomorrowWelcome /></ClientProtected>} />
    ...
    <Route path="/minha-area/notificacoes" element={<ClientProtected label="alertas"><MyTomorrowNotifications /></ClientProtected>} />
  ...
  </Routes><SaveCatalogRadarButton /><AnalyticsProvider /><InstallPrompt /><FloatingButtons /></BrowserRouter></TooltipProvider></QueryClientProvider>
);
export default App;
