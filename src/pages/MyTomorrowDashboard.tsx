...
import { CalendarDays, ChevronRight, Compass, Loader2, MapPinned, PlaneTakeoff, Plus, Sparkles } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!session) return;
        const [{ data: profile }, allTrips] = await Promise.all([
  ...
  return (
    <div className="min-h-screen bg-[#041012] text-white">
      ...
      <main className="mx-auto max-w-6xl px-4 pb-16 pt-24 sm:px-6 lg:px-8">
        ...
        {!loading && error ? <div className="mt-6 rounded-2xl border border-amber-300/20 bg-amber-300/5 p-5 text-sm text-amber-100">A nova camada de viagens ainda depende da migration acumulada da Fase 2. Seus módulos operacionais atuais permanecem preservados.</div> : null}
