...
} from "@/components/opportunities/live/LiveRouteGlobe";
import { LiveOfferOverlay } from "@/components/opportunities/live/LiveOfferOverlay";
import { LiveSessionCockpit } from "@/components/opportunities/live/LiveSessionCockpit";
...
import { liveRoutesFromOffers } from "@/lib/liveRoute";
import { deriveLiveSessionSnapshot } from "@/lib/liveSessionModel";
import { TRAVEL_OFFERS_NOTICE } from "@/lib/travelOffersPublic";
...
const stateCopy: Record<TomorrowLiveState, { eyebrow: string; body: string }> = {
  ...
  thinking: { eyebrow: "Buscando possibilidades", body: "O Téo organiza suas preferências e consulta possibilidades reais antes de responder." },
...
function useAdaptiveMotion() {
  ...
  useEffect(() => {
    ...
    const hardwareConcurrency = capabilities.hardwareConcurrency || 8;
    const deviceMemory = capabilities.deviceMemory ?? 8;
    const saveData = capabilities.connection?.saveData === true;
    const slowConnection = capabilities.connection?.effectiveType === "2g" || capabilities.connection?.effectiveType === "slow-2g";
    setLowPerformance(hardwareConcurrency <= 4 || deviceMemory <= 4 || saveData || slowConnection || window.innerWidth < 390);
...
function latestOfferUpdate(values: Array<string | null>) {
  const validDates = values
    .filter((value): value is string => Boolean(value))
    .map((value) => new Date(value))
    .filter((value) => !Number.isNaN(value.getTime()));
  if (!validDates.length) return null;
  return new Date(Math.max(...validDates.map((value) => value.getTime()))).toISOString();
}
...
export default function OpportunitiesLive() {
  ...
  const latestInventoryUpdate = useMemo(
    () => latestOfferUpdate(offers.map((offer) => offer.updated_at)),
    [offers],
  );
  const sessionSnapshot = deriveLiveSessionSnapshot({
    status: voiceStatus,
    connected,
    online,
    offerCount: offers.length,
    handoffReady: Boolean(offerHandoff),
  ...
  return (
    <div
      ...
      data-journey-stage={sessionSnapshot.stage}
    >
      ...
      <main>
        <section className="relative isolate min-h-[calc(100svh-4.5rem)] overflow-hidden border-b border-tomorrow-line">
          ...
          <div className="mx-auto w-full max-w-[96rem] px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-6">
            <div className="mx-auto max-w-6xl text-center">
              <OpportunityBadge variant="neutral"><Sparkles className="size-3.5" aria-hidden="true" />Tomorrow Live</OpportunityBadge>
              <p className="mt-3 min-h-4 text-xs font-bold uppercase tracking-[0.22em] text-tomorrow-teal-soft transition-opacity duration-300">{copy.eyebrow}</p>
              <h1 className="mx-auto mt-2 max-w-6xl font-editorial text-4xl leading-[0.92] text-tomorrow-text sm:text-5xl lg:text-[clamp(3rem,5.8vw,5.1rem)]">{LIVE_TITLE}</h1>
              <p className="mx-auto mt-3 max-w-3xl text-sm leading-relaxed text-tomorrow-muted transition-opacity duration-300 sm:text-base">{copy.body}</p>
            </div>

            <div className="relative mx-auto mt-1 w-full max-w-[72rem]">
              <LiveRouteGlobe
                ...
                onMicrophoneClick={globeMicrophoneAction}
                className="max-w-[68rem] [&>div]:min-h-[22rem] sm:[&>div]:min-h-[29rem] lg:[&>div]:min-h-[36rem]"
              />
              ...
              <div className="pointer-events-none absolute left-[5%] top-[57%] hidden items-center gap-2 rounded-full border border-tomorrow-teal/30 bg-tomorrow-background/72 px-3 py-1.5 text-[0.67rem] font-semibold text-tomorrow-teal-soft backdrop-blur sm:flex" aria-hidden="true">
                ...
                {primaryRoute?.origin.label ?? "Diga sua origem"}
              </div>
              <div className="pointer-events-none absolute right-[5%] top-[38%] hidden items-center gap-2 rounded-full border border-tomorrow-gold/35 bg-tomorrow-background/72 px-3 py-1.5 text-[0.67rem] font-semibold text-tomorrow-gold-soft backdrop-blur sm:flex" aria-hidden="true">
                ...
                {primaryRoute?.destination.label ?? "Diga seu destino"}
              </div>
            </div>
